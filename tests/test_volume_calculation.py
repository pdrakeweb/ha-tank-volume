...
def test_ellipsoidal_head_volume_formula():
    ...
    # V = (π × a / (3 × r²)) × h² × (3r - h)
    expected = (math.pi * head_depth / (3.0 * radius * radius)) * fill_height * fill_height * (3.0 * radius - fill_height)
...
def test_tank_with_heads_2_1_ellipsoidal_half():
    ...
    assert result is not None
    # With ellipsoidal heads, at h = diameter/2, volume is less than 50%
    # because the heads have less volume in the lower half
